...
from ..packaging.metadata import load_package_meta
...
class PackageInfo:
    """Information about a package to be released."""

    def __init__(self, path: Path, meta: PackageMeta | ExtensionMeta):
...
def discover_packages(formulas_dir: Path | str, single_formula: bool = False) -> List[PackageInfo]:
    ...
    if single_formula or (formulas_dir / "FORMULA").exists() or (formulas_dir / "EXTENSION").exists():
        ...
        try:
            ...
            package_type = detect_package_type(formulas_dir)
            if not _is_valid_package_layout(formulas_dir, package_type):
                print(f"Warning: Invalid {package_type} package layout in {formulas_dir}")
                return formulas

            formulas.append(PackageInfo(formulas_dir, meta))
    ...
    for item in formulas_dir.iterdir():
        ...
        if not (item / "FORMULA").exists() and not (item / "EXTENSION").exists():
            continue
        ...
        try:
            ...
            package_type = detect_package_type(item)
            if not _is_valid_package_layout(item, package_type):
                print(f"Warning: Invalid {package_type} package layout in {item}")
                continue

            formulas.append(PackageInfo(item, meta))
...
def is_new_version(formula: PackageInfo, index: Index | None) -> bool:
                ...
                for formula in released:
                    ...
                    if archive_path.exists():
                        ...
                        entry = IndexEntry(
                            ...
                            sources=[formula.meta.source] if formula.meta.source else [],
                            dependencies=formula.meta.dependencies,
                            type=detect_package_type(formula.path),
                        )
...
FormulaInfo = PackageInfo
discover_formulas = discover_packages
release_formulas = release_packages
