"""Sync package modules to Salt cache."""
...
def sync(ctx, cache_dir):
    """Sync vendor package modules to Salt's extension modules cache.

    Copies custom modules (_modules, _states, _grains, etc.) from vendor
    packages to Salt's extmods cache directory, making them available to Salt.
    ...
    try:
        ...
        if not cache_dir:
            ...
            from salt_bundle.salt.loader import _find_project_config
