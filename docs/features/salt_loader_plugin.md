...
salt-bundle registers a comprehensive loader plugin via Python entry points. The module `salt_bundle.salt.loader` is declared as a `salt.loader` entry point, which Salt discovers at startup. This module exposes one function per Salt module type: `module_dirs()`, `states_dirs()`, `grains_dirs()`, `returner_dirs()`, `runner_dirs()`, `pillar_dirs()`, `engines_dirs()`, `proxy_dirs()`, `beacons_dirs()`, `render_dirs()`, `utils_dirs()`, `fileserver_dirs()`, and over a dozen others — covering every extensible subsystem in Salt. Each function scans the vendor directory for formula subdirectories matching `_<type>/` and returns their absolute paths. Results are cached with `functools.lru_cache` keyed by formula type. The `fileserver_dirs()` hook additionally injects the `bundlefs` backend from inside the `salt_bundle` package itself. No manual Salt configuration is required beyond listing `salt_bundle` in the entry point registry.
