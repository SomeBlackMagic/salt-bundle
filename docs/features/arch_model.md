...
salt-bundle is a **package manager with a dependency resolution system**, implemented as a standalone Python CLI tool (`salt-bundle`). The model is project-centric: a project directory contains `Saltfile` (`SaltfileConfig`), which declares package dependencies and optional `index.yaml` sources. Resolved dependencies are downloaded as `.tgz` archives, verified by SHA256 digest, and unpacked into a local `vendor/` directory. The tool is fully decoupled from the Salt master: it runs locally (or in CI) and produces a vendor tree that Salt consumes through its loader plugin. Package metadata is defined in `FORMULA` and `EXTENSION` files.
