...
The source for salt-bundle packages is **`.tgz` (gzipped tar) archives**. Archives are hosted in repositories that serve an `index.yaml` file (an `Index` Pydantic model with `apiVersion`, `generated`, and a `packages` map). Repositories can be HTTP/HTTPS URLs, `file://` URLs, or local directory paths. The `index.yaml` stores, per package version, the archive URL, SHA256 digest, package type, and the dependency list extracted from package metadata.
