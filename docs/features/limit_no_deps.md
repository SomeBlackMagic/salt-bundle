...
salt-bundle implements a full dependency system. Dependencies are declared in `Saltfile` as a list of objects with `name`, optional version constraint, and optional `source` repository containing `index.yaml`. `SaltfileConfig` validates the format on load. During `salt-bundle project update`, the resolver queries repository indexes, evaluates constraints via `dependencies.resolver.resolve_version()`, and writes pinned results to `Saltfile.lock`.
