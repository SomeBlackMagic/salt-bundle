...
salt-bundle artifacts are immutable by construction. Each `.tgz` package, once published to a repository and indexed, is identified by its SHA256 digest (`sha256:<hex>`) stored in `index.yaml`. The lock file (`Saltfile.lock`) records this digest alongside the exact version, repository URL, archive path, and package type. A given lock file entry therefore refers to a specific, content-addressed artifact: any change to the package content produces a different digest and fails verification. The download cache is keyed by digest (`~/.cache/salt-bundle/packages/<sha256hex>.tgz`), preventing two distinct archives from sharing a cache entry.
