...
The **local project directory** is the control center. The `Saltfile` file in the project root defines all repositories and direct dependencies. The `salt-bundle` CLI tool (run by a developer, CI pipeline, or deployment script) reads this file and writes `Saltfile.lock` with resolved versions. There is no Salt master involvement at this stage — the tool operates independently and produces a vendor tree that can be used on any machine. User-level configuration (`~/.config/salt-bundle/config.yaml`, a `UserConfig` model) can add global repositories and an `allowed_repos` whitelist for security enforcement.
